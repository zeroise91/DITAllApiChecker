...
void Accelerometer_item_fill_cb(void *data, Evas_Object *obj, void *event_info) {
	...
	elm_list_item_append(list, notification_list[0].name, NULL, NULL,
			_sensorcallback_layout_fill_cb2depth, &notification_list[0]);
...
void LinearAccelation_item_fill_cb(void *data, Evas_Object *obj,
		void *event_info) {
	elm_list_item_selected_set(event_info, EINA_FALSE);
	notification_data* notify_info = (notification_data*) data;
	layout_view_data2 *datas = calloc(1, sizeof(layout_view_data2));
...
Accelerometer Accelerometer_get_Instance() {
	...
	(pAccelerometer == NULL) ?
			pAccelerometer = NewAccelerometer() : pAccelerometer;
...
LinearAccelation LinearAccelation_get_Instance() {
	...
	(LinearAccelation == NULL) ?
			LinearAccelation = NewLinearAccelation() : LinearAccelation;
...
RotationVector RotationVector_get_Instance() {
	...
	(RotationVector == NULL) ?
			RotationVector = NewRotationVector() : RotationVector;
...
void accCALLBACK(sensor_h sensor, sensor_event_s *event, void *user_data) {
	...
	snprintf(data->result_text, 1024, "timestamp: %llu<br>"
			...
			"x: %f<br>"
			"y: %f<br>"
			"z: %f<br>", event->timestamp, event->value_count, event->accuracy,
	...
	if (pA->isSupported(pA)) {
		...
		if (b) {
			snprintf(data->result_text, 1024,
					"ACCELEROMETER add callback completed");
		} else {
			snprintf(data->result_text, 1024,
					"ACCELEROMETER add callback fail");
		}
...
void accdatachCallback(notification_data *data) {
	...
	if (pA->isSupported(pA)) {
		...
		if (b) {
			snprintf(data->result_text, 1024,
					"ACCELEROMETER detach callback completed");
		} else {
			snprintf(data->result_text, 1024,
					"ACCELEROMETER detach callback fail");
		}
	} else {
		snprintf(data->result_text, 1024,
				"device doesn't support ACCELEROMETER");
...
void GravitydatachCallback(notification_data *data) {
	...
	if (pA->isSupported(pA)) {
		...
		if (b) {
			snprintf(data->result_text, 1024,
					"Gravity detach callback completed");
		} else {
...
void LinearAccelationCALLBACK(sensor_h sensor, sensor_event_s *event,
		void *user_data) {
...
void LinearAccelationaddCallback(notification_data *data) {
	...
	if (pA->isSupported(pA)) {
		...
		if (b) {
			snprintf(data->result_text, 1024,
					"LinearAccelation add callback completed");
		} else {
			snprintf(data->result_text, 1024,
					"LinearAccelation add callback fail");
		}

	} else {
		snprintf(data->result_text, 1024,
				"device doesn't support LinearAccelation");
	}
}
void LinearAccelationdatachCallback(notification_data *data) {
	...
	if (pA->isSupported(pA)) {
		...
		if (b) {
			snprintf(data->result_text, 1024,
					"LinearAccelation detach callback completed");
		} else {
			snprintf(data->result_text, 1024,
					"LinearAccelation detach callback fail");
		}
	} else {
		snprintf(data->result_text, 1024,
				"device doesn't support LinearAccelation");
...
void MagnetoMeterCALLBACK(sensor_h sensor, sensor_event_s *event,
		void *user_data) {
...
void MagnetoMeteraddCallback(notification_data *data) {
	...
	if (pA->isSupported(pA)) {
		...
		if (b) {
			snprintf(data->result_text, 1024,
					"MagnetoMeter add callback completed");
		} else {
...
void MagnetoMeterdatachCallback(notification_data *data) {
	...
	if (pA->isSupported(pA)) {
		...
		if (b) {
			snprintf(data->result_text, 1024,
					"MagnetoMeter detach callback completed");
		} else {
			snprintf(data->result_text, 1024,
					"MagnetoMeter detach callback fail");
		}
	} else {
		snprintf(data->result_text, 1024,
				"device doesn't support MagnetoMeter");
...
void RotationVectorCALLBACK(sensor_h sensor, sensor_event_s *event,
		void *user_data) {
...
void RotationVectoraddCallback(notification_data *data) {
	...
	if (pA->isSupported(pA)) {
		...
		if (b) {
			snprintf(data->result_text, 1024,
					"RotationVector add callback completed");
		} else {
			snprintf(data->result_text, 1024,
					"RotationVector add callback fail");
		}

	} else {
		snprintf(data->result_text, 1024,
				"device doesn't support RotationVector");
	}
}
void RotationVectordatachCallback(notification_data *data) {
	...
	if (pA->isSupported(pA)) {
		...
		if (b) {
			snprintf(data->result_text, 1024,
					"RotationVector detach callback completed");
		} else {
			snprintf(data->result_text, 1024,
					"RotationVector detach callback fail");
		}
	} else {
		snprintf(data->result_text, 1024,
				"device doesn't support RotationVector");
	...
	if (pA->isSupported(pA)) {
		...
		if (vs.accuracy != 0) {
			snprintf(data->result_text, 1024, "timestamp: %llu<br>"
					...
					"y: %f<br>"
					"z: %f<br>"
					"w: %f<br>", vs.timestamp, vs.value_count, vs.accuracy,
					vs.x, vs.y, vs.z, vs.w);
		} else {
			snprintf(data->result_text, 1024, "Sensor is off");
		}
...
void OrientationCALLBACK(sensor_h sensor, sensor_event_s *event,
		void *user_data) {
...
void OrientationaddCallback(notification_data *data) {
	...
	if (pA->isSupported(pA)) {
		...
		if (b) {
			snprintf(data->result_text, 1024,
					"Orientation add callback completed");
		} else {
...
void OrientationdatachCallback(notification_data *data) {
	...
	if (pA->isSupported(pA)) {
		...
		if (b) {
			snprintf(data->result_text, 1024,
					"Orientation detach callback completed");
		} else {
			snprintf(data->result_text, 1024,
					"Orientation detach callback fail");
		}
	} else {
		snprintf(data->result_text, 1024, "device doesn't support Orientation");
	}
...
void GyroscopeaddCallback(notification_data *data) {
	...
	if (pA->isSupported(pA)) {
		...
		if (b) {
			snprintf(data->result_text, 1024,
					"Gyroscope add callback completed");
		} else {
...
void GyroscopedatachCallback(notification_data *data) {
	...
	if (pA->isSupported(pA)) {
		...
		if (b) {
			snprintf(data->result_text, 1024,
					"Gyroscope detach callback completed");
		} else {
...
void LightCALLBACK(sensor_h sensor, sensor_event_s *event, void *user_data) {
	...
	snprintf(data->result_text, 1024, "timestamp: %llu<br>"
			"value_count: %d<br>"
			"accurancy: %d<br>"
			"level: %f<br>", event->timestamp, event->value_count,
			event->accuracy, event->values[0]);
...
void LightdatachCallback(notification_data *data) {
	...
	if (pA->isSupported(pA)) {
		...
		if (b) {
			snprintf(data->result_text, 1024,
					"Light detach callback completed");
		} else {
	...
	if (pA->isSupported(pA)) {
		...
		if (vs.accuracy != 0) {
			snprintf(data->result_text, 1024, "timestamp: %llu<br>"
					"value_count: %d<br>"
					"accurancy: %d<br>"
					"level: %f<br>", vs.timestamp, vs.value_count, vs.accuracy,
					vs.level);
		} else {
			snprintf(data->result_text, 1024, "Sensor is off");
		}
...
void ProximityCALLBACK(sensor_h sensor, sensor_event_s *event, void *user_data) {
	...
	snprintf(data->result_text, 1024, "timestamp: %llu<br>"
			...
			"proximity: %f<br>", event->timestamp, event->value_count,
			event->accuracy, event->values[0]);
...
void ProximityaddCallback(notification_data *data) {
	...
	if (pA->isSupported(pA)) {
		...
		if (b) {
			snprintf(data->result_text, 1024,
					"Proximity add callback completed");
		} else {
...
void ProximitydatachCallback(notification_data *data) {
	...
	if (pA->isSupported(pA)) {
		...
		if (b) {
			snprintf(data->result_text, 1024,
					"Proximity detach callback completed");
		} else {
	...
	if (pA->isSupported(pA)) {
		...
		if (vs.accuracy != 0) {
			snprintf(data->result_text, 1024, "timestamp: %llu<br>"
					"value_count: %d<br>"
					"accurancy: %d<br>"
					"proximity: %f<br>", vs.timestamp, vs.value_count,
					vs.accuracy, vs.proximity);
		} else {
			snprintf(data->result_text, 1024, "Sensor is off");
		}
	} else {
...
void PressureCALLBACK(sensor_h sensor, sensor_event_s *event, void *user_data) {
	...
	snprintf(data->result_text, 1024, "timestamp: %llu<br>"
			"value_count: %d<br>"
			"Pressureurancy: %d<br>"
			"hPa: %f<br>", event->timestamp, event->value_count,
			event->accuracy, event->values[0]);
...
void PressureaddCallback(notification_data *data) {
	...
	if (pA->isSupported(pA)) {
		...
		if (b) {
			snprintf(data->result_text, 1024,
					"Pressure add callback completed");
		} else {
...
void PressuredatachCallback(notification_data *data) {
	...
	if (pA->isSupported(pA)) {
		...
		if (b) {
			snprintf(data->result_text, 1024,
					"Pressure detach callback completed");
		} else {
...
void UltraVioletCALLBACK(sensor_h sensor, sensor_event_s *event,
		void *user_data) {
	...
	snprintf(data->result_text, 1024, "timestamp: %llu<br>"
			...
			"uvindex: %f<br>", event->timestamp, event->value_count,
			event->accuracy, event->values[0]);
...
void UltraVioletaddCallback(notification_data *data) {
	...
	if (pA->isSupported(pA)) {
		...
		if (b) {
			snprintf(data->result_text, 1024,
					"UltraViolet add callback completed");
		} else {
...
void UltraVioletdatachCallback(notification_data *data) {
	...
	if (pA->isSupported(pA)) {
		...
		if (b) {
			snprintf(data->result_text, 1024,
					"UltraViolet detach callback completed");
		} else {
			snprintf(data->result_text, 1024,
					"UltraViolet detach callback fail");
		}
	} else {
		snprintf(data->result_text, 1024, "device doesn't support UltraViolet");
	}
	...
	if (pA->isSupported(pA)) {
		...
		if (vs.accuracy != 0) {
			snprintf(data->result_text, 1024, "timestamp: %llu<br>"
					"value_count: %d<br>"
					"accurancy: %d<br>"
					"uvindex: %f<br>", vs.timestamp, vs.value_count,
					vs.accuracy, vs.uvindex);
		} else {
			snprintf(data->result_text, 1024, "Sensor is off");
		}
	} else {
...
void TemperatureCALLBACK(sensor_h sensor, sensor_event_s *event,
		void *user_data) {
	...
	snprintf(data->result_text, 1024, "timestamp: %llu<br>"
			"value_count: %d<br>"
			"accurancy: %d<br>"
			"°C: %f<br>", event->timestamp, event->value_count, event->accuracy,
			event->values[0]);
...
void TemperatureaddCallback(notification_data *data) {
	...
	if (pA->isSupported(pA)) {
		...
		if (b) {
			snprintf(data->result_text, 1024,
					"Temperature add callback completed");
		} else {
...
void TemperaturedatachCallback(notification_data *data) {
	...
	if (pA->isSupported(pA)) {
		...
		if (b) {
			snprintf(data->result_text, 1024,
					"Temperature detach callback completed");
		} else {
			snprintf(data->result_text, 1024,
					"Temperature detach callback fail");
		}
	} else {
		snprintf(data->result_text, 1024, "device doesn't support Temperature");
	}
	...
	if (pA->isSupported(pA)) {
		...
		if (vs.accuracy != 0) {
			snprintf(data->result_text, 1024, "timestamp: %llu<br>"
					"value_count: %d<br>"
					"accurancy: %d<br>"
					"°C: %f<br>", vs.timestamp, vs.value_count, vs.accuracy,
					vs.celsius);
		} else {
			snprintf(data->result_text, 1024, "Sensor is off");
		}
...
void HumidityCALLBACK(sensor_h sensor, sensor_event_s *event, void *user_data) {
	...
	snprintf(data->result_text, 1024, "timestamp: %llu<br>"
			"value_count: %d<br>"
			"accurancy: %d<br>"
			"percent: %f<br>", event->timestamp, event->value_count,
			event->accuracy, event->values[0]);
...
void HumidityaddCallback(notification_data *data) {
	...
	if (pA->isSupported(pA)) {
		...
		if (b) {
			snprintf(data->result_text, 1024,
					"Humidity add callback completed");
		} else {
...
void HumiditydatachCallback(notification_data *data) {
	...
	if (pA->isSupported(pA)) {
		...
		if (b) {
			snprintf(data->result_text, 1024,
					"Humidity detach callback completed");
		} else {
