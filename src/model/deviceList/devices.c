...
void devicestatus_item_fill_cb(void *data, Evas_Object *obj, void *event_info) {
	...
	datas->layout = ui_utils_layout_add(datas->navi, _layout_view_destroy2,
			datas);
	...
	elm_list_item_append(list, notification_list[0].name, NULL, NULL,
			display_view_layout_fill_cb, &notification_list[0]);
	elm_list_item_append(list, notification_list[1].name, NULL, NULL,
			battery_view_layout_fill_cb, &notification_list[1]);
	elm_list_item_append(list, notification_list[2].name, NULL, NULL,
			flash_view_layout_fill_cb, &notification_list[2]);
	elm_list_item_append(list, notification_list[3].name, NULL, NULL,
			vibrator_item_fill_cb, &notification_list[3]);

	evas_object_hide(
			elm_object_part_content_unset(datas->layout,
					"elm.swallow.content"));
	elm_object_part_content_set(datas->layout, "elm.swallow.content", list);
	evas_object_show(list);

	datas->navi_item = elm_naviframe_item_push(datas->navi, datas->name, NULL,
			NULL, datas->layout, NULL);
}
...
void preference_item_fill_cb(void *data, Evas_Object *obj, void *event_info) {
	...
	for (int i = 0; i < size; i++) {
		...
		notification_list[i].status = lists;
		elm_list_item_append(list, notification_list[i].name, NULL, NULL,
				_tab_view_layout_fill_cb2depth, &notification_list[i]);
...
void file_item_fill_cb(void *data, Evas_Object *obj, void *event_info) {
	...
	if (img == NULL) {
		...
		evas_object_image_file_set(img,
				"/opt/usr/apps/org.tizen.ditapichecker.shared/res/images/test.jpg",
				NULL);//Set the source file from where an image object must fetch the real image data
	...
	elm_list_item_append(list, notification_list[0].name, NULL, NULL,
			file_view_layout_fill_cb, &notification_list[0]);
	elm_list_item_append(list, notification_list[1].name, NULL, NULL,
			video_view_layout_fill_cb, &notification_list[1]);
	elm_list_item_append(list, notification_list[2].name, NULL, NULL,
			audio_view_layout_fill_cb, &notification_list[2]);
	elm_list_item_append(list, notification_list[3].name, NULL, NULL,
			image_view_layout_fill_cb, &notification_list[3]);
...
void sensor_item_fill_cb(void *data, Evas_Object *obj, void *event_info) {
	...
	elm_list_item_append(list, notification_list[0].name, NULL, NULL,
			Accelerometer_item_fill_cb, &notification_list[0]);
	elm_list_item_append(list, notification_list[1].name, NULL, NULL,
			Gravity_item_fill_cb, &notification_list[1]);
	elm_list_item_append(list, notification_list[2].name, NULL, NULL,
			LinearAccelation_item_fill_cb, &notification_list[2]);
	elm_list_item_append(list, notification_list[3].name, NULL, NULL,
			MagnetoMeter_item_fill_cb, &notification_list[3]);
	elm_list_item_append(list, notification_list[4].name, NULL, NULL,
			RotationVector_item_fill_cb, &notification_list[4]);
	elm_list_item_append(list, notification_list[5].name, NULL, NULL,
			Orientation_item_fill_cb, &notification_list[5]);
	elm_list_item_append(list, notification_list[6].name, NULL, NULL,
			Gyroscope_item_fill_cb, &notification_list[6]);
	elm_list_item_append(list, notification_list[7].name, NULL, NULL,
			Light_item_fill_cb, &notification_list[7]);
	elm_list_item_append(list, notification_list[8].name, NULL, NULL,
			Proximity_item_fill_cb, &notification_list[8]);
	elm_list_item_append(list, notification_list[9].name, NULL, NULL,
			Pressure_item_fill_cb, &notification_list[9]);
	elm_list_item_append(list, notification_list[10].name, NULL, NULL,
			UltraViolet_item_fill_cb, &notification_list[10]);
	elm_list_item_append(list, notification_list[11].name, NULL, NULL,
			Temperature_item_fill_cb, &notification_list[11]);
	elm_list_item_append(list, notification_list[12].name, NULL, NULL,
			Humidity_item_fill_cb, &notification_list[12]);
...
void mediarecorder_item_fill_cb(void *data, Evas_Object *obj, void *event_info) {
	...
	elm_list_item_append(list, notification_list[0].name, NULL, NULL,
			camerarecorder_view_layout_fill_cb, &notification_list[0]);
	elm_list_item_append(list, notification_list[1].name, NULL, NULL,
			audiorecorder_view_layout_fill_cb, &notification_list[1]);
...
void camerarecorder_view_layout_fill_cb(void *data, Evas_Object *obj,
		void *event_info) {
	elm_list_item_selected_set(event_info, EINA_FALSE);
	notification_data* notify_info = (notification_data*) data;
	layout_view_data2 *datas = calloc(1, sizeof(layout_view_data2));
...
void audiorecorder_view_layout_fill_cb(void *data, Evas_Object *obj,
		void *event_info) {
	elm_list_item_selected_set(event_info, EINA_FALSE);
	notification_data* notify_info = (notification_data*) data;
	layout_view_data2 *datas = calloc(1, sizeof(layout_view_data2));
...
void isCharging(notification_data* data) {
	...
	snprintf(data->result_text, 1024, "%s",
...
void extractinfo(notification_data* data) {
	...
	if (image->setURI(image, "/opt/usr/media/Images/test.jpg")) {
		snprintf(data->result_text, 1024,
				"image <br>/opt/usr/media/Images/test.jpg<br>metadata extract completed");
	} else {
		snprintf(data->result_text, 1024,
				"image <br>/opt/usr/media/Images/test.jpg<br>metadata extract failed");
	}
}
...
void getimageMediaId(notification_data* data) {
	...
	snprintf(data->result_text, 1024, "MediaID: %s",
			str ? str : "<br>press setURI button first");

}
void getimageDateTaken(notification_data* data) {
	Image image = image_get_instance();
	String str = image->getDate(image);
	snprintf(data->result_text, 1024, "date: %s",
			str ? str : "<br>press setURI button first");
...
void getimageWidth(notification_data* data) {
	...
	if (width != -1) {
		...
		snprintf(data->result_text, 1024, "press setURI button first");
...
void deletefile(notification_data* data) {
	...
	if (file->Delete("/opt/usr/media/Downloads/copied.jpg") == true) {
		...
		sprintf(data->result_text,
				"/opt/usr/media/Downloads/copied.jpg<br>deleted");
	} else {
		sprintf(data->result_text,
				"delete target File:<br>/opt/usr/media/Downloads/copied.jpg<br>doesn't exist");
	}
	DestroyFile(file);
}
void copyfile(notification_data* data) {
	...
	if (file->Copy(srcfilepath, "/opt/usr/media/Downloads/copied.jpg") == true) {
		...
		sprintf(data->result_text, "copy source file:<br>%s doesn't exist",
				srcfilepath);
	}
	free(srcfilepath);
	DestroyFile(file);
...
void movefile(notification_data* data) {
	File file = NewFile();
	if (file->Move("/opt/usr/media/Downloads/copied.jpg",
			"/opt/usr/media/Downloads/moved.jpg")) {
		;
		sprintf(data->result_text,
				"/opt/usr/media/Downloads/copied.jpg<br><br>moved to<br><br>/opt/usr/media/Downloads/moved.jpg");
	} else {
		sprintf(data->result_text,
				"/opt/usr/media/Downloads/copied.jpg doesn't exist");
...
void playvideo(notification_data* data) {
	...
	if (((VideoExtends*) v)->uri && ((VideoExtends*) v)->EvasObject) {
		...
		sprintf(data->result_text,
				"press<br>setEvasObject button<br>setURI(Video) button<br>first");
	}
}
...
void setURI_video(notification_data* data) {
	...
	String videofilepath = getSharedResourceFile(
			"videos/Samsung Galaxy S3 Theme song.mp4");
...
void preference_int_get(notification_data* data) {
	...
	sprintf(data->result_text, " %d get by<br>keyword:\"int\" at preference",
			a);
}
void preference_string_get(notification_data* data) {
	...
	sprintf(data->result_text, " %s get by<br>keyword:\"String\" at preference",
			x);
	free(x);
	x = NULL;
...
void preference_boolean_get(notification_data* data) {
	...
	sprintf(data->result_text,
			"\"%s\" get by<br>keyword:\"boolean\" at preference",
			b ? "true" : "false");
}
void preference_double_get(notification_data* data) {
	...
	sprintf(data->result_text,
			" %lf get by<br>keyword:\"double\" at preference", d);
}
...
void preference_string_set(notification_data* data) {
	...
	sprintf(data->result_text, "&lt;\"%s\",\"%s\"&gt;<br> saved at preference",
			"string", buffer);
...
void preference_boolean_set(notification_data* data) {
	...
	sprintf(data->result_text,
			" &lt; \"%s\",\"%s\"&gt;<br> saved at preference", "boolean",
			b ? "true" : "false");
...
void preference__remove(notification_data* data) {
	...
	if (pref->Remove("double")) {
		strcat(str, "double <br>");
	}

	if (pref->Remove("boolean")) {
		strcat(str, "boolean <br>");
	}
...
void preference__clear(notification_data* data) {
	...
	if (pref->Clear()) {
		...
		sprintf(data->result_text, "clear failed");
...
void camerarecorderinit(notification_data* data) {
	...
	bool b = cr->Init(cr, "/opt/usr/media/Videos/VIDEO_RECORDED.mp4",
			CAMERA_BACK, g_eo);

	sprintf(data->result_text, "%s",
			b ? "video will recorded at <br>/opt/usr/media/Videos/VIDEO_RECORDED.mp4" : "init failed");
}
...
void camerarecordercancel(notification_data* data) {
	...
	sprintf(data->result_text, "%s",
			b ? "cancel sucess<br>VIDEO will not recorded" : "cancel fail");
	if (b == true) {
		evas_object_hide(g_eo);
...
void audiorecorderinit(notification_data* data) {
	...
	sprintf(data->result_text, "%s",
			b ? "AUDIO will recorded at <br>/opt/usr/media/Sounds/AUDIO_RECORDED.mp4" : "init failed");
...
void audiorecorderend(notification_data* data) {
	...
	bool b = pa->End(pa);
	sprintf(data->result_text, "%s",
			b ? "AUDIO recorded at<br>/opt/usr/media/Sounds/AUDIO_RECORDED.mp4" : "AudioRecorder  end fail");
...
void audiorecordercancel(notification_data* data) {
	...
	bool b = pa->Cancel(pa);
	sprintf(data->result_text, "%s",
			b ? "cancel sucess<br>AUDIO will not recorded" : "cancel fail");
